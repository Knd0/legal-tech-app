...
| **Facturas y Auditorías**| **100%** | Paginación server-side. Facturación habilitada para transacciones de tipo `PAGO`. Generación de PDF oficial Factura C en backend (`pdfkit`) con estética *Organic*. Descarga directa protegida y envío automático al cliente vía Email (Resend) y WhatsApp (documento adjunto vía Baileys). |
...
- **Estado de Bot de WhatsApp en Perfil**: La interfaz de configuración de alertas de WhatsApp (`ProfileComponent`) utiliza el signal `whatsappBotReady` y sondeos dinámicos (`startQrPolling`) para reflejar el estado del bot en tiempo real. Para evitar estados inconsistentes (como mostrar "Bot Conectado" cuando la sesión en el backend se ha cerrado), la UI valida que el bot esté listo en el servidor (`status.ready`). Si se detecta desconexión, la UI expone el contenedor de vinculación (código QR o código de emparejamiento por teléfono). El sondeo se inicia inmediatamente al activar el interruptor de WhatsApp.
- **Conexión de WhatsApp basada en Baileys**: Migramos la integración a la librería `@whiskeysockets/baileys` para evitar el uso de Puppeteer/Chromium en el servidor. Esto reduce drásticamente el consumo de memoria (~30-50MB) y elimina definitivamente los crashes por falta de recursos (`Killed`) en contenedores con 512MB RAM (Railway).
- **Persistencia de Sesión de Baileys**: La sesión de Baileys consta de archivos JSON locales en `whatsapp-auth/` (o en la carpeta del usuario en Windows). Estos archivos se sincronizan de forma transparente bidireccionalmente con la tabla `whatsapp_sessions` en la base de datos PostgreSQL, tanto en el inicio (`syncDbToLocalFolder`) como en cada actualización de credenciales/claves (`syncLocalFolderToDb`).
- **Gestión de Notificaciones de WhatsApp por Rol**: La vinculación por código QR/teléfono de la sesión central de WhatsApp está disponible exclusivamente para usuarios `ADMIN`. Los endpoints de actualización global de `/settings` están protegidos con `@Roles('ADMIN')`. Los abogados (`USER`) ven una interfaz simplificada que les permite activar o desactivar alertas de WhatsApp en sus propios números de teléfono verificados, con soporte de repeticiones (1, 2, 3 o 4 veces al día distribuidas uniformemente a lo largo de las 24 horas usando un cron por hora) y hora de inicio personalizada. El bot central de la plataforma les envía los recordatorios de vencimiento directamente a su número verificado.
- **Configuración de Alertas Personalizada**: Los abogados pueden personalizar exhaustivamente sus alertas de vencimiento. La interfaz del Perfil permite activar/desactivar canales (mensajería directa por WhatsApp y notificaciones push en navegador), definir alertas en momentos con doble anticipación en días configurables (`daysBeforeAlert` y `daysBeforeAlertSecondary`), habilitar notificaciones en el mismo día del vencimiento y aplicar filtros selectivos por tipo de evento (audiencias, plazos procesales y escritos). Toda esta lógica está vinculada reactivamente a la base de datos y es interpretada dinámicamente en el cron job diario del backend.
