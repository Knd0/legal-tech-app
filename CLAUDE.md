...
- **Storage efímero en Railway**: Cloudinary handles document uploads. WhatsApp auth session is now persisted in PostgreSQL via a custom `WhatsappDbStore` with `RemoteAuth` under key `RemoteAuth-themis-session`, solving the Railway ephemeral restarts issue. Puppeteer cache generation is disabled (`--disk-cache-size=1`, etc.) to keep the session ZIP tiny (~1.5MB), avoiding Railway event loop freezes and 504 Gateway Timeouts during LEVEL-9 RemoteAuth zipping. *Local Dev & Windows Sandbox Bypass:* Local Windows Puppeteer launches store session data inside `~/themis-whatsapp-auth` (instead of inside the hidden `.gemini` folder structure) to prevent browser sandbox execution blocks that crash with `Navigating frame was detached`. In local development (when `NODE_ENV !== 'production'`), the WhatsApp client automatically initializes on boot and runs in headful mode (`headless: false`), which pops open a browser window displaying the WhatsApp Web QR code so the developer can scan it directly in the browser window.
...
- **Bypass de Bug de Código de Emparejamiento en whatsapp-web.js**: La función `requestPairingCode` de `whatsapp-web.js` falla con `TypeError: window.onCodeReceivedEvent is not a function` si el cliente no se inicializó con la opción `pairWithPhoneNumber` en el constructor. Lo solucionamos exponiendo dinámicamente `window.onCodeReceivedEvent` en el contexto del navegador (Puppeteer) antes de llamar a la función interna.
